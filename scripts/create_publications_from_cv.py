...
def read_tsv(filepath):
    ...
    try:
        with open(filepath, 'r', encoding='utf-8') as f:
            ...
            for row in reader:
                # Clean up any empty string values and strip whitespace
                cleaned_row = {}
...
"""
    ...
    if image_mapping:
        ...
        for img_title, img_data in image_mapping.items():
            ...
            if similarity > best_similarity and similarity > 0.6:  # Lower threshold
        ...
        if best_match:
            ...
            print(f"  Matched image for '{title[:50]}...' (similarity: {best_similarity:.2f})")
...
def main():
    """Create publication files from CV data"""
    
    # Paths
    cv_master_dir = "/root/clawd/data/cv-master"
    output_dir = Path("/tmp/academic-site/content/publications")
    
    print("=== Creating Publications from CV Data ===")
    
    # Read CV papers data
    cv_papers = read_tsv(f"{cv_master_dir}/cv-papers.tsv")
    ...
    print(f"\n=== Complete! Created {created_count} publication files ===")
    return True
